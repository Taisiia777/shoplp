...
//
//     return (
//         <div className={`relative content-center self-stretch mt-[30px] ${isMobile ? 'h-[400px] mt-[2rem]' : 'h-[36.25rem]'}`}>
...
//                             {/* Фоновое изображение */}
//                             <div
//                                 className={`absolute inset-0 bg-[url('/images/img_banner_slider.png')] bg-contain bg-center bg-no-repeat z-0`}
...
export default function HomeBannerSection() {
    ...
    const slides = isMobile
        ? [
            ...
            {
                image: "/images/img_slider_1_375.png",
                text: "",
                customTextStyles: "text-left px-4 whitespace-pre-line mt-[20vh] ml-[1rem] text-[1.5rem]",
                mobileBackgroundSize: "cover"
            },
        ...
        : [
            {
                image: "/images/img_banner_slider.png",
                text: "Создай свой уютный момент",
                customTextStyles: "mt-[35vh] text-[3rem] mb-[20vh] ml-auto mr-auto",
                desktopBackgroundSize: "contain"
            },
            {
                image: "/images/img_slider_2.png",
                text: "Худи на все\nслучаи жизни",
                customTextStyles: "text-left px-4 whitespace-pre-line mt-[30vh] ml-[1rem] text-[3rem]",
                desktopBackgroundSize: "contain"
            },
        ];
        ...
        <div className={`relative content-center self-stretch ${isMobile ? 'h-[400px] mt-[2rem]' : 'h-[36.25rem]'}`}>
            <div className="MainSlider container-xs mx-auto flex w-full md:px-[1.25rem] relative z-10">
                <Slider
                    ...
                    renderDotsItem={(props) => (
                        <div
                            className={`inline-block h-[4px] ${isMobile ? "w-[80px]" : "w-[100px]"} rounded-full cursor-pointer mr-2 ${
                                props.isActive ? "bg-white" : "bg-gray-400"
                            }`}
                        />
                    )}
                    ...
                    items={slides.map((slide, index) => (
                        ...
                        >
                            ...
                            <div
                                ...
                                style={{
                                    ...
                                    backgroundSize: isMobile
                                        ? slide.mobileBackgroundSize || 'cover'
                                        : slide.desktopBackgroundSize || 'contain',
                            ...
                            {/* Градиентный слой поверх изображения */}
                            <div
                                className="absolute inset-0 bg-gradient-to-t from-[#5A4A43] max-h-[70%] mt-[40%] to-transparent opacity-90 rounded-b-[20px] z-10 pointer-events-none max-w-[1212px] mx-auto"/>

                            {/* Контент слайдера */}
                            <div
                                className={`relative z-20 flex ${index === 0 ? 'items-center' : 'items-start'} text-center w-full ${isMobile ? 'mt-10 gap-6' : 'mt-[6.13rem] gap-[11.25rem]'}`}
                            >
                                <Text
                                    size="1280_h1"
                                    as="p"
                                    className={`font-roundsblack text-white ${isMobile ? 'text-2xl leading-snug' : 'text-[2.75rem]'} font-normal tracking-[0.06rem] px-4 ${
                                        slide.customTextStyles
                                    }`}
                                >
                                    {slide.text}
